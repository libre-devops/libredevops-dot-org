...
| where StdDev > 0
| extend ZScore = (AvgBytes - StdDev) / StdDev
```
...
| summarize
    ...
    AvgInterval = avg(totimespan(TimeGenerated - prev(TimeGenerated))),
    StdDevInterval = stdev(totimespan(TimeGenerated - prev(TimeGenerated)))
  by DeviceName, RemoteIP, RemotePort
| where ConnectionCount > 20
| where StdDevInterval < 30s  // very regular = suspicious
```
...
SecurityAlert
| where TimeGenerated > ago(30d)
| summarize Count = count() by bin(TimeGenerated, 1d), SystemAlertId, ProductName
| render timechart
```
...
DeviceNetworkEvents
| where TimeGenerated > ago(1d)
| join kind=inner BlockedDomains on $left.RemoteUrl contains $right.Domain
```
