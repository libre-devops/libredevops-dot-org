...
# Level word is colourised (like PowerShell colours its Write-* streams), but
# only when it will actually render - so piped into a ticket or a file the
# output stays plain, with no raw ANSI escapes.
# ---------------------------------------------------------------------------
