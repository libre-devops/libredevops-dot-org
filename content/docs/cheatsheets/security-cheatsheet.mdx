...
```powershell
[System.Environment]::GetEnvironmentVariables() | Sort-Object Name
```
...
if (token) {
  const [h, p] = token.split('.').slice(0,2).map(b => JSON.parse(atob(b.replace(/-/g,'+').replace(/_/g,'/'))));
...
```bash
grep -rn --include="*.{js,ts,py,go,rb,php,env,yaml,yml,json,toml,tf,sh}" \
