...
**Security Copilot Custom Agents** - Build domain-specific security agents
- Use cases: SOC-specific triage (org's SIEM rules), incident response workflows, compliance checks
- Configuration: Define agent persona, grounding data, and tool access in Azure portal or via API
- Grounding with Log Analytics: Connect to Log Analytics workspace; agent queries KQL natively
- Example agent: "Threat hunter" that correlates events across Log Analytics, Sentinel, Defender
- Access: Security Copilot portal → Custom Agents → Create with KQL grounding, Sentinel playbooks, custom rules
...
- **Using API keys instead of managed identity** - Keys can be stolen, rotated manually
- **Trusting AI outputs without verification** - Hallucinations happen; validate before apply
- **Unlimited tool access** - Agent should only call what it needs
- **No audit logs** - Cannot investigate incidents or prove compliance
- **Hard-coded system prompts in code** - Changes require code deploy; use configuration
- **Ignoring token costs** - Runaway agents can cost thousands per day
- **Mixing providers in same app** - OpenAI in some places, Azure OpenAI in others; use one
- **No fallback strategy** - If primary AI call fails, entire feature breaks
- **Accepting all prompt arguments as-is** - Validate argument types and ranges
