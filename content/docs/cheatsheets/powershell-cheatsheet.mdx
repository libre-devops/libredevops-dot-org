...
description: Advanced PowerShell 7+ patterns for Azure, DevOps, scripting, Pester testing, REST APIs, and automation.
---

# PowerShell Cheat Sheet

PowerShell 7+ patterns for Azure automation, DevOps pipelines, scripting, and general systems administration. All examples are cross-platform unless noted.
...
### Managed identity (IMDS / workload identity)
...
function Write-Log {
    ...
    if ($Properties) { $Properties.GetEnumerator() | ForEach-Object { $entry[$_.Key] = $_.Value } }

    switch ($Level) {
        'WARN'  { Write-Warning  ($entry | ConvertTo-Json -Compress) }
        'ERROR' { Write-Error    ($entry | ConvertTo-Json -Compress) }
        'DEBUG' { Write-Debug    ($entry | ConvertTo-Json -Compress) }
        default { Write-Host     ($entry | ConvertTo-Json -Compress) }
    }
}

Write-Log 'Deployment started' -Properties @{ Environment = 'prod'; Region = 'uksouth' }
```
...
$modules = @(
    @{ Name = 'Az';                     RequiredVersion = '12.0.0' }
