...
description: Advanced Azure CLI, Bicep, IAM, networking, AKS, monitoring, policy, and governance patterns for Azure platform engineers.
---

# Azure Cheat Sheet

Azure CLI, Bicep, and platform engineering patterns. Covers auth, IAM, compute, networking, AKS, storage, monitoring, policy, and governance. Most CLI examples require `az` 2.55+ and `jq`.
...
# Create SP with auto-generated secret (legacy shortcut)
...
# Add federated credential (GitHub Actions OIDC)
...
```bash
# Create (with RBAC authorization model)
az keyvault create \
  --name              kv-myapp-prod \
  --resource-group    rg-myapp-prod-uksouth \
  --location          uksouth \
...
# Admin credentials (bypass RBAC — emergency only)
