...
- **`any` type** - silently disables type checking for that value and everything downstream. Use `unknown` and narrow before use, or define the actual shape with an interface or type.
...
- **`console.log` in server-side code** ⚠️ - synchronous, unstructured, and bypasses redaction. Use pino or another structured logger with levels.
