...
apply_to_resources() {
    ...
    for resource in $resources; do
        echo "Applying $operation to $resource..."
        eval "az $resource_type $operation --name $resource --resource-group $resource_group"
    done
}

# Usage: apply_to_resources "my-rg" "Microsoft.Storage/storageAccounts" "update --set kind=StorageV2"
```
...
blue_green_deploy() {
    ...
    local current_slot=$(az appservice web config show --name "$app_name" \
        --query "swapSlotsConfiguration.slotSwapConfiguration" -o tsv)
    local target_slot=$([[ "$current_slot" == "staging" ]] && echo "production" || echo "staging")
    
    echo "Deploying $new_version to $target_slot..."
    
    # Deploy to inactive slot
    az webapp deployment slot create --name "$app_name" --slot "$target_slot" \
        --configuration-source "$current_slot" || true
    
    # Push code
    git push "https://$app_name.scm.azurewebsites.net:443/$app_name.git" HEAD:"refs/heads/main"
    
    # Wait for deployment
    sleep 30
    
    # Health check before swap
    local target_url="https://$app_name-$target_slot.azurewebsites.net$health_endpoint"
    if ! curl -sf "$target_url" > /dev/null; then
        echo "✗ Health check failed on $target_slot, aborting swap"
        return 1
    fi
    
    echo "✓ Health check passed, swapping slots..."
    az appservice web deployment slot swap --name "$app_name" \
        --slot "$target_slot"
...
# Usage
blue_green_deploy "my-api-app" "v2.1.0" "/health"
```
