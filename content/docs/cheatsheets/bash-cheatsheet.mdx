...
PowerShell colours its `Write-Warning` / `Write-Error` / `Write-Verbose` streams by default; Bash doesn't, but you can. The trick is to emit ANSI **only when it'll be rendered**: drop a colour map and a guard in alongside the weights, then wrap the level word in the `printf`.
...
# ...then swap the printf in _log to wrap the level in colour/reset:
printf '%s  %s%-5s%s  %s\n' \
  "$(date -u +'%Y-%m-%dT%H:%M:%SZ')" \
  "${_LOG_COLOURS[$level]:-}" "$level" "$_LOG_RESET" \
  "$msg" >&2
```
