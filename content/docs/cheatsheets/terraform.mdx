...
description: Advanced Terraform reference for Azure - provider auth, state, lifecycle, expressions, validation, modules, RBAC, Key Vault, and CI/CD patterns.
---
...
#### Managed Identity (recommended for self-hosted runners)
...
#### Service principal with client secret (legacy - avoid in new projects)
...
terraform_run() {
  ...
  command -v tfenv &>/dev/null && { tfenv install "$tf_version"; tfenv use "$tf_version"; }
...
### Ansible provider - inventory management + playbook execution

The `ansible.com/ansible` provider manages **Ansible inventory state** - it registers hosts and groups so that the Ansible CLI (or AWX/AAP) can discover them. Running the playbook itself is triggered via `terraform_data` with a `local-exec` provisioner, which executes after the inventory is registered.

```hcl
terraform {
  required_providers {
    azurerm = { source = "hashicorp/azurerm", version = "~> 4.0" }
    ansible = { source = "ansible.com/ansible", version = "~> 1.3" }
  }
}
...
# --- Trigger playbook after inventory is registered ---

resource "terraform_data" "run_playbook" {
  # Re-run the playbook whenever the VM's IP or the playbook file changes
  triggers_replace = {
    host_ip         = azurerm_linux_virtual_machine.vm.public_ip_address
    playbook_sha    = filesha256("${path.module}/playbooks/configure_web.yml")
  }

  provisioner "local-exec" {
    command = <<-EOT
      ansible-playbook \
        -i "${azurerm_linux_virtual_machine.vm.public_ip_address}," \
        --private-key ~/.ssh/id_rsa \
        --ssh-extra-args="-o StrictHostKeyChecking=no" \
        -e "env=${var.environment}" \
        -e "vm_name=${azurerm_linux_virtual_machine.vm.name}" \
        ${path.module}/playbooks/configure_web.yml
    EOT
  }

  depends_on = [ansible_host.vm]
}
```

### Why `terraform_data` instead of `null_resource`

`terraform_data` (Terraform 1.4+) is the built-in replacement for `null_resource`. It needs no provider, behaves identically for `triggers_replace` and `provisioner`, and avoids requiring the `hashicorp/null` provider.

```hcl
# Old pattern - requires null provider
resource "null_resource" "run_playbook" {
  triggers = { host = azurerm_linux_virtual_machine.vm.public_ip_address }
  provisioner "local-exec" { command = "..." }
}

# Modern pattern - built-in, no extra provider
resource "terraform_data" "run_playbook" {
  triggers_replace = { host = azurerm_linux_virtual_machine.vm.public_ip_address }
  provisioner "local-exec" { command = "..." }
}
```
...
When deploying several VMs with `for_each`, create one `terraform_data` trigger per host using the same key:
...
resource "terraform_data" "configure_vms" {
  for_each = azurerm_linux_virtual_machine.vms

  triggers_replace = {
    host_ip      = each.value.public_ip_address
    playbook_sha = filesha256("${path.module}/playbooks/configure_web.yml")
  }
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ansible-playbook \
        -i "${each.value.public_ip_address}," \
        --private-key ~/.ssh/id_rsa \
        --ssh-extra-args="-o StrictHostKeyChecking=no" \
        -e "vm_name=${each.value.name}" \
        ${path.module}/playbooks/configure_web.yml
    EOT
  }

  depends_on = [ansible_host.vms[each.key]]
}
```
