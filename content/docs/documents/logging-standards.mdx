...
2. **One JSON object per line.** Emit newline-delimited JSON (NDJSON / `jsonl`) to `stdout`. One event = one line = one parse.
...
## Reference implementation - `Write-LdoLog` (PowerShell)
...
- [LibreDevOpsHelpers on the PowerShell Gallery](https://www.powershellgallery.com/packages/LibreDevOpsHelpers) - the reference implementation
- [PowerShell Standards](/docs/documents/powershell-standards) - native streams, PSFramework, OTel tracing, Azure telemetry sync
- [Python Standards](/docs/documents/python-standards) · [Bash Standards](/docs/documents/bash-standards) · [CI/CD Standards](/docs/documents/cicd-standards) - logging in context
