...
**Considerations:**
- Requires Sentinel connection (configured separately in Azure portal or via `azurerm_logic_app_connection`)
...
resource "azurerm_logic_app_connection" "sentinel" {
...
resource "azurerm_logic_app_trigger_custom" "sentinel_incident" {
  ...
  body = jsonencode({
    inputs = {
      host = {
        connection = {
          name = azurerm_logic_app_connection.sentinel.id
        }
      }
      path = "/incident"
      queries = {
...
resource "azurerm_logic_app_connection" "service_bus" {
...
resource "azurerm_logic_app_trigger_custom" "service_bus_trigger" {
  ...
  body = jsonencode({
    inputs = {
      host = {
        connection = {
          name = azurerm_logic_app_connection.service_bus.id
        }
      }
      method = "GET"
...
resource "azurerm_logic_app_connection" "event_grid" {
  name                = "eventgrid-${var.environment}"
  resource_group_name = azurerm_resource_group.this.name
  managed_api_id      = "/subscriptions/${data.azurerm_client_config.current.subscription_id}/providers/Microsoft.Web/locations/${var.location}/managedApis/azureeventgrid"
}
...
resource "azurerm_logic_app_trigger_custom" "event_grid_trigger" {
  ...
  body = jsonencode({
    inputs = {
      host = {
        connection = {
          name = azurerm_logic_app_connection.event_grid.id
        }
      }
...
5. **Custom triggers - use connection references** - Never hardcode credentials; reference `azurerm_logic_app_connection` resources
...
resource "azurerm_logic_app_action_http" "kql_query" {
  ...
  logic_app_id = azurerm_logic_app_standard.this.id
  method       = "POST"
  uri          = "https://${var.log_analytics_workspace_name}.api.loganalytics.io/v1/workspaces/${azurerm_log_analytics_workspace.this.id}/query"
...
4. **Connection references** - Always reference `azurerm_logic_app_connection` resources, never hardcode connection IDs:
...
```hcl
resource "azurerm_logic_app_action_http" "initialize_variables" {
...
resource "azurerm_logic_app_action_http" "webhook_call" {
  ...
  depends_on = [azurerm_logic_app_action_http.initialize_variables]
}
```
...
**Considerations:**
- Connector must be pre-registered as `azurerm_logic_app_connection`
...
resource "azurerm_logic_app_connection" "jira" {
  name                = "jira-${var.environment}"
  resource_group_name = azurerm_resource_group.this.name
  managed_api_id      = "/subscriptions/${data.azurerm_client_config.current.subscription_id}/providers/Microsoft.Web/locations/${var.location}/managedApis/jira"
}
...
resource "azurerm_logic_app_action_custom" "create_jira_ticket" {
  ...
  body = jsonencode({
    inputs = {
      host = {
        connection = {
          name = azurerm_logic_app_connection.jira.id
        }
      }
      method   = "post"
...
resource "azurerm_logic_app_action_scope" "scope_enrichment" {
  ...
  actions = jsonencode({
    ...
    "Update_incident_tags" = {
      ...
      inputs = {
        host = {
          connection = { name = azurerm_logic_app_connection.sentinel.id }
        }
        method = "patch"
...
resource "azurerm_logic_app_action_scope" "scope_error_handler" {
  ...
  actions = jsonencode({
    "Log_error_to_law" = {
      ...
      inputs = {
        host = {
          connection = { name = azurerm_logic_app_connection.log_analytics.id }
        }
        method = "post"
    ...
    "Alert_team_enrichment_failure" = {
      ...
      inputs = {
        host = {
          connection = { name = azurerm_logic_app_connection.teams.id }
        }
        method = "post"
        path   = "/webhooks"
        body = {
          text = "⚠️ Enrichment failed for incident @{variables('incident_id')} (@{variables('severity')}). Check logs."
        }
      }
      runAfter = {
  ...
  depends_on = [azurerm_logic_app_action_scope.scope_enrichment]
}
```
...
resource "azurerm_logic_app_action_condition" "severity_check" {
  depends_on = [azurerm_logic_app_action_http.initialize_variables]
  # ...
}

resource "azurerm_logic_app_action_for_each" "notify_recipients" {
  depends_on = [azurerm_logic_app_action_condition.severity_check]
  # ...
}
```
