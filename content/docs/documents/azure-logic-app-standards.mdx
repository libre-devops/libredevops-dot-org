...
Trigger and action names are the workflow's display surface: run history, error messages, and `runAfter` edges all show them. Be verbose - a name should read as a sentence describing the event, with underscores for spaces (the designer renders them as spaces).
...
module "monitor_alerts" {
  ...
  metric_alerts = {
    ...
    "Playbook_failures_-_${name}_runs_failing" => {
      scopes           = [id]
